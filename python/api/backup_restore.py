...
from python.helpers.backup import BackupService, ResilientRestoreService
...
class BackupRestore(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        # Enable error-correcting restore by default
        use_error_correction = (
            request.form.get("use_error_correction", "true").lower() == "true"
        )
        ...
        try:
            # Use error-correcting restore service
            if use_error_correction:
                resilient_service = ResilientRestoreService()
                result = await resilient_service.restore_with_error_correction(
